...
public class TelemetryReportingConfigurationServiceTest {

   private TelemetryReportingConfigurationService service;
   ...
   @Before
   public void setup() {
      service = new TelemetryReportingConfigurationService();
      service.setSystemDescriptorService(sdService);
   }

   @Test
   public void testTelemetryReportingConfiguration() {
      ...
      IProperty configProperty = getMockedRestTelemetryReportingConfiguration(rate, address, interfaceName, port, path,
               contentType, method);
      ...
      Collection<TelemetryReportingConfiguration> configurations = service.getConfigurations(options, model);
   ...
   private static IProperty getMockedRestTelemetryReportingConfiguration(int rate, String address, String interfaceName,
            int port,
      ...
      when(property.getReferencedDataType().getFullyQualifiedName()).thenReturn(
               TelemetryReportingConfigurationService.REST_TELEMETRY_REPORTING_CONFIGURATION_QUALIFIED_NAME);
      when(property.getData().isSet()).thenReturn(true);
      when(property.getData().getReferencedDataType().getFullyQualifiedName())
               .thenReturn(
                        TelemetryReportingConfigurationService.REST_TELEMETRY_REPORTING_CONFIGURATION_QUALIFIED_NAME);
      IDataField rateField = mock(IDataField.class);
      when(property.getData().getFieldByName(TelemetryReportingConfigurationService.RATE_FIELD_NAME))
               .thenReturn(Optional.of(rateField));
      when(property.getData().getPrimitive(rateField).getInteger()).thenReturn(BigInteger.valueOf(rate));
      IDataField configField = mock(IDataField.class);
      when(property.getData().getFieldByName(TelemetryReportingConfigurationService.CONFIG_FIELD_NAME)).thenReturn(
