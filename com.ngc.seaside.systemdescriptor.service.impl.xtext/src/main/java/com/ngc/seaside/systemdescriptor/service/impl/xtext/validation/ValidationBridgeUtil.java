...
public class ValidationBridgeUtil {
	 ...
	 *
	 * @param object
	 *            the source object on which the method is called
	 * @param xtext
	 *            the raw, unwrapped XText type that corresponds to the type
	 *            that is being wrapped
	 * @param method
	 *            the method that was called on the object
	 * @return the structural feature
	 * @throws IllegalValidationDeclarationException
	 *             if their is no structural feature associated with the method.
	 *             This usually happens if the user calls a method that returns
	 *             a derived value or if the method returns an object that is no
	 *             owned by the object the {@link ProxyingValidationContext} was
	 *             created for.
	 */
	public static EStructuralFeature getFeature(Object object, EObject xtext, Method method) {
		...
		if (object instanceof IPrimitiveDataField) {
			return doGetFeature((IPrimitiveDataField) object, xtext, method);
		}
		if (object instanceof IReferencedDataField) {
			return doGetFeature((IReferencedDataField) object, xtext, method);
		}
	...
	private static EStructuralFeature doGetFeature(IPrimitiveDataField object, EObject xtext, Method method) {
		switch (method.getName()) {
		case "getType":
	...
	private static EStructuralFeature doGetFeature(IReferencedDataField object, EObject xtext, Method method) {
		switch (method.getName()) {
		case "getData":
