...
public class BootstrapCommandProvider implements IBootstrapCommandProvider {
	...
	public void run(String[] arguments) {
		...
		IParameterCollection parameters = parameterService
				.parseParameters(Arrays.asList(Arrays.copyOfRange(arguments, 1, arguments.length)));
	 ...
	 */
	protected IBootstrapCommandOptions createBootstrapCommandOptions(IParameterCollection userInputParameters,
	 ...
	 *         the templateContent.
	 */
	protected IParameterCollection unpackTemplate(IBootstrapCommand command,
		...
		if (templateService.templateExists(templatePrefix)) {
			try {
				...
				logService.trace(getClass(), "Unpacking templateContent for '%s' to '%s'", userSuppliedParameters,
						outputPath);
				ITemplateOutput templateOutput = templateService.unpack(templatePrefix, userSuppliedParameters,
						outputPath, false);

				return convertParameters(templateOutput, outputPath);
			} catch (TemplateServiceException e) {
				logService.error(getClass(), e, "Unable to unpack the templateContent for command'%s'. Aborting",
						command);
			}
		}
		return null;
