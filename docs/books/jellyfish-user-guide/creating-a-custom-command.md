...
`ICommand` interface are commands that _do not require a System Descriptor_ in order to be run.  Conversely, commands
that implement `IJellyfishCommand` do require a System Descriptor.  Most of the the, commands implement the
`IJellyfishCommand` interface.

## Types of Commands
Various abstract base classes are provided to make implementing commands easier.  Generic commands can extend the
class `AbstractJellyfishCommand`.  This includes command that generate code, documentation, etc.

Commands may also perform various type of analysis and may report any findings.  These findings are consolidated into
...
The `NAME` of the command is what users will use to run the command.  For example, a user would run this command like
`jellyfish example-command`.  

`createUsage` it where the command describes itself.  The usage object includes a description of the command and the
parameters that commands supports.  Each parameter has a name, a descriptor, and may be required or optional.  This 
information is used when showing help to the user.

`doRun()` is where the actual logic of the command resides.  The `getOptions()` method 
...
Users can alos use the `IJellyFishCommandOptions` to retrieve arguments provided by the user.  For example, 
  ...
  protected void analyzeModel(IModel model) {
    if (model.getOutputs().isEmpty()) {
      ...
      SystemDescriptorFinding<?> finding =
        ExampleFindingTypes.NO_OUTPUTS.createFinding(message, location, 1);
...
This analysis checks models to ensure models have at least one output field declared.  It does this by overriding the
method `analyzeModel`.  Analysis command can also override `analyzeData` and `analyzeEnumeration` to review data types
and enumerations.
...
`src/main/resources/docs/no-inputs.md`.  This file might contain content like this:
...
## Registing the Analysis Command
Like any other type of command, the command must be registered in a Guice module:
...
jellyfish analyze \ 
 ...
 gav=mmy.group:my.sd.project:1.0.0 
```
